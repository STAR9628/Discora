-- Migration (RESTORED): 20260925175657_public_data_api_rate_limits
--
...
create table if not exists private.data_api_rate_limits (
  key_hash text not null,
  scope text not null,
  bucket_kind text not null,
  bucket_start timestamptz not null,
  request_count integer not null default 0,
  primary key (key_hash, scope, bucket_kind)
);

alter table private.data_api_rate_limits enable row level security;
revoke all on schema private from public, anon, authenticated;
revoke all on private.data_api_rate_limits from public, anon, authenticated;

create table if not exists private.data_api_rate_limit_secret (
  id boolean primary key default true check (id),
  secret bytea not null
);

revoke all on private.data_api_rate_limit_secret from public, anon, authenticated;

insert into private.data_api_rate_limit_secret (id, secret)
...
declare
  ...
  secret_value bytea;
begin
  if req_method in ('GET', 'HEAD') or req_method = '' then
    return;
  end if;
  ...
  key_hash := encode(
    digest(raw_ip || ':' || encode(secret_value, 'hex'), 'sha256'),
    'hex'
  );
