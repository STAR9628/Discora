-- Sprint 9 Fix: Grant search_content() RPC execute to anon & return room_type (ADR-025 / AUD-001)

drop function if exists public.search_content(text, integer, integer);
...
-- Grant execute to authenticated, anon, and public
grant execute on function public.search_content(text, integer, integer) to authenticated, anon, public;

comment on function public.search_content(text, integer, integer) is
  'Full-text search across rooms, messages, claims, evidence, questions (ADR-025 / AUD-001). Granted to anon & authenticated. Returns room_type for clean UI routing.';
