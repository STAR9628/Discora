-- Fix Data API rate-limit pre-request hook.
-- The previous function used a PL/pgSQL variable named bucket_kind while also
-- referencing the target table's bucket_kind in ON CONFLICT. PostgreSQL treated
-- the conflict target as ambiguous under the live PostgREST request path,
-- producing 42702, which Discora mapped to a permission error.
-- Keep the rate-limit design unchanged; only rename the local variable.
