-- Phase 4B: Fix join_with_access_code rate limiting
--
-- PostgreSQL rolls back the entire transaction when an exception is raised,
-- including any INSERTs that occurred earlier in the same transaction.
-- The previous implementation inserted failure records and then raised an
-- exception, causing the counter to never persist.
--
-- This migration replaces the function to return null on invalid code
-- instead of raising an exception. The failure record now persists because
