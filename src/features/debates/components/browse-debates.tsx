...
export function BrowseDebates() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Sort — uniform control geometry so short labels (e.g. "Newest")
          carry the same visual weight as longer ones. Wraps on narrow screens. */}
      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
        <span className="font-semibold uppercase tracking-wider">Sort by:</span>
        {(["most_active", "most_evidence", "most_participants", "newest", "recently_updated"] as const).map((s) => (
          <button
            key={s}
            onClick={() => setSort(s)}
            aria-pressed={sort === s}
            className={`inline-flex min-h-9 min-w-32 items-center justify-center whitespace-nowrap rounded-lg px-3 py-1 text-xs font-semibold border transition-all duration-150 active:scale-[0.96] cursor-pointer ${
              sort === s
                ? "border-primary/40 bg-primary/5 text-primary"
                : "border-border text-muted-foreground hover:text-foreground"
            }`}
          >
            {s === "most_active" ? "Most Active" : s === "most_evidence" ? "Most Evidence" : s === "most_participants" ? "Most Participants" : s === "newest" ? "Newest" : "Recently Updated"}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        statusFilter === "closing_soon" ? (
          <div className="rounded-2xl border border-dashed border-border bg-card/20 p-12 text-center max-w-xl mx-auto space-y-4">
            ...
            <div>
              <h2 className="text-lg font-bold">No debates are closing soon</h2>
              <p className="text-sm text-muted-foreground mt-1.5">
                Debates with a participation deadline in the next 7 days will appear here.
              </p>
            </div>
          </div>
        ) : (
          <div className="rounded-2xl border border-dashed border-border bg-card/20 p-12 text-center max-w-xl mx-auto space-y-4">
            ...
            <div>
              <h2 className="text-lg font-bold">No debates found</h2>
              <p className="text-sm text-muted-foreground mt-1.5">
              ...
              <div className="space-y-2.5 sm:space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {debate.status === "active" && (
                    <span className="rounded-full bg-blue-500/10 px-2.5 py-0.5 sm:px-3 sm:py-1 text-[10px] font-bold uppercase tracking-wider text-blue-400 border border-blue-500/30">
                      Active
                    </span>
                  )}
                  ...
                  {debate.closesAt && (
                    <span className="flex items-center gap-1 text-[11px] sm:text-xs text-muted-foreground">
                      ...
                      <span>Closes {formatDate(debate.closesAt, { month: "short", day: "numeric" })}</span>
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-4 text-xs">
                  <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-2.5 sm:p-3 space-y-1">
                    <span className="font-extrabold text-blue-400 uppercase tracking-wider text-[11px] sm:text-xs">Proposition</span>
                    <p className="text-[11px] sm:text-xs text-muted-foreground italic leading-relaxed line-clamp-2">
                      &ldquo;{room.title}&rdquo;
                    </p>
                    <p className="font-semibold text-blue-400 text-[11px] sm:text-xs">Supports motion</p>
                  </div>
                  <div className="rounded-lg border border-slate-500/20 bg-slate-500/5 p-2.5 sm:p-3 space-y-1">
                    <span className="font-extrabold text-slate-400 uppercase tracking-wider text-[11px] sm:text-xs">Opposition</span>
                    <p className="text-[11px] sm:text-xs text-muted-foreground italic leading-relaxed line-clamp-2">
                      &ldquo;{room.title}&rdquo;
                    </p>
                    <p className="font-semibold text-slate-400 text-[11px] sm:text-xs">Challenges motion</p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-[11px] sm:text-xs text-muted-foreground pt-0.5 sm:pt-1">
